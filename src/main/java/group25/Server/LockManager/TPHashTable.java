...
public class TPHashTable
{
	...
	// if called with a TransactionLockObject - hashCode is just the transaction id m_xid
	// If called with a DataLockObject - hashCode is m_data.hashCode() -> native Java method
	// if called with a WaitLockObject - hashCode is m_data.hashCOde() -> native Java method

	// Every time LockManager.Lock() is called, a TransactionLockObject and a DataLockObject is added
	// vectSlot (which is a vector) can have all TransactionLockObjects with m_xid = x, other TransactionLockObjects which can hash to that same spot, 
	// or DataLockObjects/WaitLockObjects whose m_data.hashCode() maps to the same spot
	...
	// If called with a DataLockObject or WaitLockObject, get all DataLockObjects with m_data = x;
	// If called with a TransactionLockObject, gets all transactions which hash to a certain spot
