		...
		for (int i = 0; i < size; i++)
		{
			...
			{    
				...
				else if (dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_WRITE)
				{
					...
					// check whether any other DataLockObjects in the vector hold a shared lock on the data item
					else if (l_dataLockObject.getLockType() == TransactionLockObject.LockType.LOCK_READ) {
						for (int j = i+1; j<size; j++) {
							DataLockObject potential_conflict_object = (DataLockObject)vect.elementAt(j);
							if (potential_conflict_object.getXId() != dataLockObject.getXId() &&
								potential_conflict_object.getLockType() == TransactionLockObject.LockType.LOCK_READ) {
								return true;
							}
						}
